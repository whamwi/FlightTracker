-- Retire the estimated departure once the actual one is known, and keep the day-early arrival
-- repair from 11 Aug (this replaces the whole function, so both rules live here).
--
-- The est_dep rule is the mirror of the est_arr one and its reasoning is identical: an estimate
-- is superseded by an actual rather than forgotten. Missing it left 101 departed flights carrying
-- a stale estimate. XY591 on 12 Aug had real_dep 10:11:28 beside est_dep 09:59:00 — a round-minute
-- value from before the flight left, twelve minutes earlier than the departure it was estimating,
-- and the mobile card rendered it as the departure time.
--
...
begin
  ...
  on conflict (flight_date, iata_number, dep_iata, arr_iata) do update set
    ...
    -- Retired once the actual departure is known — the mirror of the est_arr rule below, and
    -- missing it left every departed flight carrying a stale estimate. XY591 on 12 Aug had
    -- real_dep 10:11:28 beside est_dep 09:59:00: a round-minute value from before the flight
    -- left, twelve minutes earlier than the departure it was estimating.
