 ...
 * stays fixed.
 *
 * "Fly" is brand gold, "Syria" the same deep green as the globe. Both are literals rather
 * than palette references because each page defines its own local `C` object with slightly
 * different values — the brand should not shift depending on which page you are looking at.
 */

const GOLD  = '#988561'
const GREEN = '#054239'
// The mark's paper plane keeps the paler wheat it was drawn with; only the wordmark moves
// to the deeper gold, which needs the contrast that a filled shape does not.
const PLANE = '#B9A779'
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 100 100" fill="none">
      ...
      <g transform="translate(58 4) rotate(-12) scale(1.7)">
        <path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z" fill={PLANE} stroke={GREEN} strokeWidth="1.1" strokeLinejoin="round" strokeLinecap="round"/>
      </g>
    </svg>
  )
}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
      ...
      <span style={{ font: `700 ${fontSize}px/1 'Instrument Sans',system-ui`, letterSpacing: '-.01em', whiteSpace: 'nowrap' }}>
        <span style={{ color: GOLD }}>Fly</span><span style={{ color: GREEN }}>Syria</span>
      </span>
    </div>
  )
}
