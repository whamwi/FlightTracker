  ...
  rows = flights
    ...
    .map(f => {
      ...
      return {
        ...
        // Omit null actuals — merge-duplicates would overwrite confirmed PF times
        ...(actualDep  ? { actual_dep_utc:  new Date(actualDep).toISOString(),  dep_delay_min: delayMin(schedDep, actualDep) }  : {}),
        ...(revisedDep ? { revised_dep_utc: new Date(revisedDep).toISOString() } : {}),
        ...(actualArr  ? { actual_arr_utc:  new Date(actualArr).toISOString(),  arr_delay_min: delayMin(schedArr, actualArr) }  : {}),
