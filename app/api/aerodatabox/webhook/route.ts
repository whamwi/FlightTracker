...
function toRow(f: any, iataToCallsign: Record<string, string>): StatusRow | null {
  ...
  return {
    ...
    // Omit actual/revised times when null so merge-duplicates doesn't overwrite
    // an existing confirmed time with null from a later webhook with no data.
    ...(actualDep  ? { actual_dep_utc:    new Date(actualDep).toISOString()  } : {}),
    ...(revisedDep ? { revised_dep_utc:   new Date(revisedDep).toISOString() } : {}),
    scheduled_arr_utc: schedArr   ? new Date(schedArr).toISOString()   : null,
    ...(actualArr  ? { actual_arr_utc:    new Date(actualArr).toISOString()  } : {}),
    ...(revisedArr ? { revised_arr_utc:   new Date(revisedArr).toISOString() } : {}),
    ...(actualDep  ? { dep_delay_min:     delayMin(schedDep, actualDep) }     : {}),
    ...(actualArr  ? { arr_delay_min:     delayMin(schedArr, actualArr) }     : {}),
